...
namespace MechICQ
{
    ...
    // Типы задач главного потока
    enum GeneralTaskType
    {
        Connect
    }

    // Типы событий для главного потока
    public enum GeneralMessageType
    {
        ConnectFailed
    }

    /* Состояние подключения, хранится, чтобы знать, как обрабатывать некоторые поступающие пакеты */
    public enum ConnectStatuses
    {
        LoginServer // Подключен к серверу логина
    }

    /* Перечисление для указания, какая именно ошибка произошла при подключении */
    public enum ConnectionError
    {
        CannotConnectToLoginServer
    }
    
    /* ========== СОПУТСТВУЮЩИЕ КЛАССЫ ========== */

    /* Класс, представляющий собой пакет. При создании получает только заголовок, данные грузятся в него потом. */
    class FLAPPacket
    {
        byte[] header; // Заголовок пакета
        public byte[] body; // Тело пакета; массив создаётся только после получения заголовка

        /* Конструктор, создающий пакет по заголовку. ~!Потом!~ ему нужно передать тело пакета.
         * Если пакет не в порядке, первый байт заголовка устанавливается в 0x00.
         * TODO: Сделать exception. */
        public FLAPPacket(byte[] headr)
        {
            if (headr.Length == 6)
                this.header = headr;
            else
                header[0] = 0x00;
            // Проверка
            if (!(header[0] == 0x2A & header[1] >= 1 & header[1] <= 5)) // Нулевой байт всегда равен 0x2A, первый - номер FLAP-канала, от 1 до 5.
                header[0] = 0x00;
            int size = (header[4] << 8) + header[5];
            body = new byte[size];
        }
        /* ФУНКЦИИ, АНАЛИЗИРУЮЩИЕ ЗАГОЛОВОК */
        public int getSize() // Возвращает размер пакета, записанный в заголовке
        {
            return
                (header[4] << 8) + header[5];
        }
        public int getSeqNum() // Возвращает sequence number пакета
        {
            return (header[2] << 8) + header[3];
        }
        public byte getChannel() // Возвращает канал, по которому пришёл пакет
        {
            return header[1];
        }
        /* ФУНКЦИЯ ЛОГГИРОВАНИЯ ПАКЕТА */
        public void log()
        {
            string buffH = "", buffA = ""; // buffH - строка с hex-представлением байт,
            // buffA - с ASCII-символами, соответствующими байтам
            ICQProtocol.log("-------------------- PACKET DUMP BEGIN", false);
            for (int i = 0; i < 6; i++)
            {
                string buff = String.Format("{0:X}", header[i]);
                buffH += (buff.Length == 1 ? "0" + buff : buff) + " ";
                buffA += System.Convert.ToChar(header[i]);
            }
            for (int i = 0; i < this.getSize(); i++)
            {
                string buff = String.Format("{0:X}", body[i]);
                buffH += (buff.Length == 1 ? "0" + buff : buff) + ((i + 6) % 16 == 0 ? "" : " ");
                buffA += System.Convert.ToChar(body[i]);
                if ((i + 6) % 16 == 0)
                    ICQProtocol.log(buffH + "  |  " + buffA, false);
            }
            // Доводим длину строки buffH до 47, дополнив её пробелами, и выводим остаток байт
            if (buffH.Length > 0)
            {
                while (buffH.Length < 47)
                    buffH += " ";
                ICQProtocol.log(buffH + "  |  " + buffA, false);
            }
            ICQProtocol.log("-------------------- PACKET DUMP END", false);
        }
    }

    /* Класс, представляющий собой общую задачу главного потока */
    class GeneralTask
    {
        protected GeneralTaskType TaskType; // Выставление типа задачи извне не допускается; это должны делать
        // конструкторы классов задач
    ...
    class ConnectTask : GeneralTask // Задача подключения
    {
        public uint uin;
        public string password;
        public ConnectTask(uint uin, string password)
        {
            this.TaskType = GeneralTaskType.Connect;
            this.uin = uin;
            this.password = password;
        }
    }

    /* Общий результат главного потока */
    public class GeneralMessage
    {
        protected GeneralMessageType MessageType;
    ...
    public class ConnectionErrorMessage : GeneralMessage
    {
        public ConnectionError errorType;
        public ConnectionErrorMessage(ConnectionError type)
        {
            this.MessageType = GeneralMessageType.ConnectFailed;
            this.errorType = type;
        }
        public string getMessageString()
        {
            switch (errorType)
            {
                case ConnectionError.CannotConnectToLoginServer:
                    return "Невозможно подключиться к серверу login.icq.com.";
                default:
    ...
    class ICQProtocol
    {
        ...
        // Потоки
        Thread generalThread; // Поток, выполняющий задачи, в т.ч. отправляет пакеты
        Thread listenThread; // Поток, "прослушивающий" сервер

        // Очередь задач главного потока
        Queue <GeneralTask> generalTasks;

        /* Очередь сообщений главного потока (для передачи ЛЮБЫХ данных модулем протокола вовне) */
        public Queue<GeneralMessage> generalMessages;

        // Механизм синхронизации потоков, задачи предоставляющих, и главного потока
        EventWaitHandle generalSem;

        /* Механизм синхронизации модуля протокола и обработчика очереди сообщений */
        public EventWaitHandle messageReady;

        // Текущий статус соединения (используется при получении от сервера некоторых инициирующих пакетов)
        ConnectStatuses connectStatus;

        // Соединение с сервером
        TcpClient serverConnection;

        // Переменная, хранящая в себе SeqNum сервера
        int serverSeqNum;
        ...
        /* Функция записи в лог-файл строки с датой */
        public static void log(string buff, bool logDate)
        // Если logDate == true, записываем дату, иначе - нет.
        {
            StreamWriter logFile = new StreamWriter(File.Open(Path.GetDirectoryName(Application.ExecutablePath) + "\\logfile.txt", FileMode.Append));
            if(logDate)
                logFile.WriteLine(String.Format("{0} : {1}",System.DateTime.Now,buff));
            else
                logFile.WriteLine(buff);
            logFile.Close();
        }
         ...
         * Инициирует семафор очереди generalSem
         * Создаёт поток-обработчик очереди задач
         * Инициализирует механизм взаимодействия с обработчиком сообщений ("семафор" messageReady) */
        {
            // Очердь сообщений нужно создать раньше - на случай, если сразу после создания очереди задач начнут поступать сообщения
            messageReady = new EventWaitHandle(false, EventResetMode.AutoReset);
            generalMessages = new Queue<GeneralMessage>();
            // Очередь задач
            generalTasks = new Queue<GeneralTask>(); 
            generalSem = new EventWaitHandle(false,EventResetMode.AutoReset);
                /* Семафор запуска процесса обработки очереди
                 * Т.е. это семафор "семафорит", когда в очереди появилось новое задание */
            // Запуск потока
            generalThread = new Thread(TaskManager);
        ...
        /* Обработчик задач - ждёт появления задач в очереди (после того, как
         * задача в очереди появилась, нужно ему об этом сообщить вызовом
         * generalSem.Set() ).
         * Работает в потоке, который инициируется конструктором. */
        void TaskManager()
        {
            while (true)
            {
                ...
                {
                    GeneralTask tmp_task;
                    lock (generalTasks)
                    {
                        tmp_task = generalTasks.Dequeue();
                    }
                    switch (tmp_task.getTaskType())
                    {
                        case GeneralTaskType.Connect:
                            ConnectTask task = (ConnectTask)tmp_task;
                            uin = task.uin;
                            password = task.password;
                            MessageBox.Show(String.Format("Connect task received, uin: {0}, password: {1}",uin,password));
                            // TODO: Перед подключением надо проконтролировать, не подключены ли мы уже - по счастливой случайности? Или не надо?
                            // РЕШИТЬ.
                            // Собственно подключаемся
                            // TODO: Реализовать возможность использования разных серверов в случае проблем
                            try
                            {
                                serverConnection = new TcpClient("login.icq.com", 5190);
                            }
                            catch (Exception)
                            {
                                ConnectionErrorMessage msg = new ConnectionErrorMessage(ConnectionError.CannotConnectToLoginServer);
                                lock (generalMessages)
                                {
                                    generalMessages.Enqueue(msg);
                                }
                                messageReady.Set();
                                break;
                            }
                            connectStatus = ConnectStatuses.LoginServer; // Это выполнится только если не произошло исключения
                            listenThread = new Thread(listener);
                            listenThread.Start();
                            // Далее для процесса коннекта нужно дождаться srv_hello.
                            // В общем, дальнейшая организация подключения гарантируется обработчиками пакетов.
                            break;
                    }
                }
            }
        ...
        /* Функция прослушивания сервера, постоянно ожидает заголовка пакета
         * После его получения создаёт новый экземпляр пакета, загружает содержимое пакета с сервера и
         * загружает его в экземпляр.
         * После распознаёт вид пакета, генерирует новое сообщение или задачу (если нужно) и семафорит кому нужно. */
        void listener()
        {
            NetworkStream server = serverConnection.GetStream();
            byte[] header = new byte[6];
            bool firstSeq = true; // После получения первого пакета эта переменная устанавливается в false;
                                // если она true, не надо контролировать SeqNum, а наоборот, его нужно получить из пакета и сохранить.
            while (true) 
            {
                server.Read(header, 0, 6); // Получаем заголовок поступившего пакета
                FLAPPacket packet = new FLAPPacket(header);
                byte[] body = new byte[packet.getSize()];
                server.Read(body, 0, packet.getSize());
                log("New packet received:", true);
                packet.log();
                // Контроль sequence number'а
                if (firstSeq)
                {
                    serverSeqNum = packet.getSeqNum();
                    firstSeq = false;
                }
                else
                    if (packet.getSeqNum() != ++serverSeqNum)
                    {
                        log(String.Format("Вместо ожидаемого SeqNum = {0} был получен {1}. Вероятна потеря пакетов.", serverSeqNum, packet.getSeqNum()), true);
                        // Что же делать дальше? Мы не можем знать, какой именно пакет мы "недополучили".
                        // TODO: Сделать соответствующее сообщение о потере серверного пакета и показать его пользователю.
                    }
                // Определение вида пакета
                switch (packet.getChannel())
                {
                    case 1: // New connection negotiation
                        if (packet.getSize() == 4 && packet.body[0] == 0x00 && // SRV_HELLO
                                                     packet.body[1] == 0x00 &&
                                                     packet.body[2] == 0x00 &&
                                                     packet.body[3] == 0x00)
                        {
                            // Получение этого пакета значит, что соединение успешно установлено и сервер ожидает наших пакетов.
                            // Если мы только что подключились к серверу логина - нужно отправлять CLI_IDENT в ответ.
                            log("SRV_HELLO packet received.", true);
                            if (connectStatus == ConnectStatuses.LoginServer)
                            {
                                // TODO: Отправить CLI_IDENT.
                            }
                        }
                        break; // Channel == 1
                }                
        ...
        /* ========== ФУНКЦИИ ДЛЯ КОНКРЕТНЫХ ЗАДАЧ ========== */
        /*      (в общем случае - по одной на задачу,
         *       предназначены для вызова извне класса)       */

        // Инициация процесса подключения (требует номер и пароль, что естественно)
        public void StartConnect(uint uin, string password)
        {
            lock (generalTasks)
            {
                ConnectTask task = new ConnectTask(uin, password);
